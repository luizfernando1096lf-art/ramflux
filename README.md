...
[![Version](https://img.shields.io/badge/version-2.47.0-blue.svg)](https://github.com/luizfernando1096lf-art/ramflux/releases)
...
Fixed deterministic crash in `PowerReadFriendlyName` (POWRPROF.dll+0x59CE) by replacing all POWRPROF calls with registry-based power plan enumeration and hardcoded GUID-to-name resolution. PowerManager background thread no longer calls POWRPROF functions periodically. Stale crash logs cleaned.
