...
);    controlLayout->addWidget(m_metricCombo);    controlLayout->addStretch();    layout->addLayout(controlLayout);    m_chart = new QChart();    m_chart->setTitle("Memory Timeline");    m_chart->setMargins(QMargins(12, 8, 12, 8));    m_chart->setBackgroundBrush(QColor("#1e1e2e"));    m_chart->setPlotAreaBackgroundBrush(QColor("#181825"));    m_chart->setPlotAreaBackgroundVisible(true);    m_chart->legend()->setVisible(true);    m_chart->legend()->setLabelColor(QColor("#cdd6f4"));    m_chart->setTitleBrush(QColor("#cdd6f4"));    m_loadSeries = new QLineSeries();    m_loadSeries->setName("Memory Load %");    QPen loadPen(QColor("#89b4fa"));    loadPen.setWidth(2);    m_loadSeries->setPen(loadPen);    m_pressureSeries = new QLineSeries();    m_pressureSeries->setName("Pressure Score");    QPen pressurePen(QColor("#f38ba8"));    pressurePen.setWidth(2);    m_pressureSeries->setPen(pressurePen);    m_overlaySeries = new QLineSeries();    QPen overlayPen(QColor("#a6e3a1"));    overlayPen.setWidth(2);    m_overlaySeries->setPen(overlayPen);
    m_eventSeries = new QScatterSeries();    m_eventSeries->setName("Events");    m_eventSeries->setMarkerSize(10);    m_eventSeries->setColor(QColor("#f9e2af"));    m_eventSeries->setBorderColor(QColor("#fab387"));
    m_chart->addSeries(m_loadSeries);    m_chart->addSeries(m_pressureSeries);    m_chart->addSeries(m_eventSeries);    m_axisX = new QDateTimeAxis();    m_axisX->setFormat("hh:mm:ss");    m_axisX->setLabelsColor(QColor("#a6adc8"));    m_axisX->setLinePenColor(QColor("#45475a"));    m_axisX->setGridLineColor(QColor("#313244"));    m_chart->addAxis(m_axisX, Qt::AlignBottom);    m_loadSeries->attachAxis(m_axisX);    m_pressureSeries->attachAxis(m_axisX);    m_eventSeries->attachAxis(m_axisX);    m_axisY = new QValueAxis();    m_axisY->setRange(0, 100);    m_axisY->setLabelsColor(QColor("#a6adc8"));    m_axisY->setLinePenColor(QColor("#45475a"));    m_axisY->setGridLineColor(QColor("#313244"));    m_chart->addAxis(m_axisY, Qt::AlignLeft);    m_loadSeries->attachAxis(m_axisY);    m_pressureSeries->attachAxis(m_axisY);    m_eventSeries->attachAxis(m_axisY);    m_chartView = new QChartView(m_chart);    m_chartView->setRenderHint(QPainter::Antialiasing);    m_chartView->setStyleSheet("background: transparent");    layout->addWidget(m_chartView, 1);
}
void HistoryChart::addDataPoint(double usedGB, double freeGB, double loadPct,                                 double pressure, double commitGB, double standbyGB) {    QDateTime now = QDateTime::currentDateTime();    m_timestamps.push_back(now);    m_usedValues.push_back(usedGB);    m_freeValues.push_back(freeGB);    m_loadValues.push_back(loadPct);    m_pressureValues.push_back(pressure);    m_commitValues.push_back(commitGB);    m_standbyValues.push_back(standbyGB);
    while(static_cast<int>(m_timestamps.size()) > m_maxPoints) {        m_timestamps.pop_front();        m_usedValues.pop_front();        m_freeValues.pop_front();        m_loadValues.pop_front();        m_pressureValues.pop_front();        m_commitValues.pop_front();        m_standbyValues.pop_front();    }    // prune old event markers outside window
    if(!m_timestamps.empty()) {
        qint64 oldest = m_timestamps.front().toMSecsSinceEpoch();
        auto pts = m_eventSeries->points();
        for(auto p : pts) if(p.x() < oldest) m_eventSeries->remove(p.x(), p.y());
    }
    updateSeries();
}
void HistoryChart::addEventMarker(const QString& label, const QDateTime& when) {
    Q_UNUSED(label);
    double y = 95.0; // top of chart
    m_eventSeries->append(when.toMSecsSinceEpoch(), y);
    if(static_cast<int>(m_eventSeries->count()) > 20) {
        auto pts = m_eventSeries->points();
        if(!pts.isEmpty()) m_eventSeries->remove(pts.first().x(), pts.first().y());
    }
}
