...
std::string name() const override;
    ...
    bool cleanStandbyList(bool force = false);
    bool gentleStandbyClean();
    bool selectiveClean();
    bool intelligentStandbyClean(const Telemetry::MemorySnapshot& snap);
    bool cleanModifiedPageList(bool force = false);
    bool trimProcesses(bool force = false);
    bool trimProcessesIdleOnly();
    bool trimColdPages();
    bool trimProcess(uint32_t pid);
    bool cleanWorkingSet();
    bool cleanFileCache(bool force = false);
    bool cleanCombinedList(bool force = false);
    bool defragment(bool force = false);
    bool quickClean();
...
int cooldownMs() const;
    ...
    void resetStats();
    void setBatteryAware(bool aware) { m_batteryAware.store(aware); }
    bool isBatteryAware() const { return m_batteryAware.load(); }
    private:
...
false};
    ...
    std::atomic<bool> m_batteryAware{true};
};
} // namespace RAMFlux::Cleaner
